...
### 현재 파일 크기: 152KB (이미지 외부 분리 후)

## 수정 이력 (2026-06-09 세션 3차)

### 콘텐츠 추가 / 수정
- **음향시스템 탭 — 배치도면(PDF p40) 추가**: "설계했습니다." 뒤 삽입, width:72% 중앙정렬
- **음향시스템 탭 — 배관배선도(PDF p62) 추가**: Crown 앰프 설명 아래 삽입, width:72%
- **음향 시뮬레이션 기준표 — 에코 유발 억제 항목 제거**
- **네트워크 탭 — Summary 토글 추가**: 오디오·컨트롤 신호(PDF p1) / 비디오 신호(PDF p2)
- **공연시스템 탭 — 티켓 가격표 추가**: A구역~스카이박스 7구역, 합계 약 15억
- **공연시스템 탭 — BEP 다이어그램 추가**: 대관료 구성 바차트 + 투자회수 시뮬레이션
  - 감가상각 10%/년 반영: 3년 15회 → **4년 19회** BEP
  - 계산식: 잔여투자비 × 1.1 − 연수익(11.5억), 4년차 19회 공연 시 BEP 달성
- **공연시스템 탭 — Summary 토글 추가**: 오디오·컨트롤 신호(PDF p3)
- **공연시스템 서브우퍼 63Hz 버튼**: flex-basis:100% (항상 새 줄 왼쪽 고정)
- **인프라 탭 텍스트 수정**: "12개 카메라의 영상 소스" → "12개의 카메라 포인트의 영상 소스"

### 버그 수정
- **모바일 이미지 확대(라이트박스) data: 조건 제거**: 외부URL 이미지도 확대 가능
- **모바일 라이트박스 중복 #lb-overlay 제거**: 일부 이미지 확대 안되던 문제
- **index.html 중복 lb-overlay 제거**: 계활 점검 중 발견·수정
- **도움사운드 로고 JPEG→PNG 복원**: filter:invert(1) 깨짐 수정, 200×200 리사이즈(10KB)
- **모바일 이미지 매핑 오류(SHA256)**: base64 앞50자 충돌 → SHA256 해시 매칭으로 수정
- **Safari 스크롤 점프 4개 함수**: perfSumToggle/netSumToggle/perfSumTab/netSumTab scrollY 복원 적용
- **공연시스템 음향시뮬레이션 버튼 width:100% 복원**
